...
void doubleStack(struct stack *stak)
{
    int *n,*t,i;
    n = (int*)malloc(sizeof(int)*stak->capacity*2);
    stak->capacity*=2;
    for(i=0;i<=stak->top;i++)
        n[i] = stak->ptr[i];
    free(stak->ptr);
    stak->ptr=n;
}

void halfStack(struct stack *stak)
{
    int *n,i;
    n = (int*)malloc(sizeof(int)*(stak->capacity/2));
    stak->capacity/=2;
...
main()
{
    ...
    while(1)
    {
        ...
        printf("capacity is %d",stak->capacity);
        getch();
